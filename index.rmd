...
dat <- read.csv("./data/output_worldwide.csv", stringsAsFactors = FALSE) 
dat_tw <- read.csv("./data/output_TW.csv", stringsAsFactors = FALSE)
r <- read.csv("./data/country_region.csv", stringsAsFactors = FALSE) %>% 
  rename("country" = "name") %>% 
  select(country, region, sub.region)
tw_county <- read.csv("./data/tw_county.csv", 
...
cbind(
  actual = ts(d$actual_cases,
              start = as.Date("2020-02-01"),
              frequency = 1),
  pred = ts(c(rep(NA, nrow(d)-1), 
              d$actual_cases[nrow(d)],
              d[nrow(d), c(4:10)] %>% unlist() %>% unname()),
            start = as.Date("2020-02-01"),
            frequency = 1)
) %>%
  dygraph() %>% 
